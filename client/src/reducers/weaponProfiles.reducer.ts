...
import {
  TOGGLE_WEAPON_PROFILE,
  EDIT_WEAPON_PROFILE,
  ADD_WEAPON_PROFILE,
  DELETE_WEAPON_PROFILE,
  MOVE_WEAPON_PROFILE,
} from '../actions/weaponProfiles.action';
import { updateItemInArray, moveItemInArray } from './helpers';

const addWeaponProfile = (state, action) => [...state, { ...action.profile, uuid: nanoid() }];

const toggleWeaponProfile = (state, action) =>
  updateItemInArray(state, action.id, profile => ({
    ...profile,
